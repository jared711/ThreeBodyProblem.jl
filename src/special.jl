...
"""
function invariant_manifolds(sys::System, rv₀, T, tf; nPts=20)
    Φ₀ = I(6)
    ...
    rvₜ = sol.u[end][1:6] # last time step
    ...
    Wsp = solve(prob_sp, TsitPap8(), reltol=myreltol)
    prob_sn = ODEProblem(CR3BPdynamics!,rv0sn, tspan_backward, sys) # stable negative
    Wsn = solve(prob_sn, TsitPap8(), reltol=myreltol)
    prob_up = ODEProblem(CR3BPdynamics!,rv0up, tspan_forward, sys) # unstable positive
    Wup = solve(prob_up, TsitPap8(), reltol=myreltol)
    prob_un = ODEProblem(CR3BPdynamics!,rv0un, tspan_forward, sys) # unstable negative
    Wun = solve(prob_un, TsitPap8(), reltol=myreltol)

    return Wsp, Wsn, Wup, Wun, Φₜ
end

function differential_corrector(sys::System, rv₀, myconst; iter=100, plot=false, t0=0., tf=1., dir=1, tol=1e-12)
   Φ₀ = I(6)
